...
#else
	.freq_responsiveness = ATOMIC_INIT(1200000),
#endif
...
#else
		if (cur_load >= inc_cpu_load && cpu_policy->cur < max_freq) {
			tmp_freq = min(cpu_policy->cur + (pump_inc_step * 100000), max_freq);
		} else if (cur_load < dec_cpu_load && cpu_policy->cur > min_freq) {
			tmp_freq = max(cpu_policy->cur - (pump_dec_step * 100000), min_freq);
		} else {
			tmp_freq = cpu_policy->cur;
		}
